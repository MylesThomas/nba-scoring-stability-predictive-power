...
date: "December 12, 2021" # "`r Sys.Date()`"
author:
  - name: Myles Thomas
    url: https://github.com/MylesThomas
...
knitr::opts_chunk$set(
  ...
  include = FALSE,
  dpi = 300
  # tidy = 'styler'  # Disabled: optional code formatting
)
...
# Project directory/file path management
# Using here::here() for cross-platform compatibility (works in RStudio and Cursor)
SCRIPT_DIR = here::here("src", "03_CODE")
PARENT_DIR = here::here("src")
INPUT_DIR = file.path(PARENT_DIR, "00_INPUT/")
INTER_DIR = file.path(PARENT_DIR, "01_INTERMEDIATE/")
OUTPUT_DIR = file.path(PARENT_DIR, "02_OUTPUT/")

print(glue::glue("current script directory: {SCRIPT_DIR}"))
print(glue::glue("parent directory: {PARENT_DIR}"))
print(glue::glue("input files directory: {INPUT_DIR}"))
print(glue::glue("intermediate files directory: {INTER_DIR}"))
print(glue::glue("output files directory: {OUTPUT_DIR}"))

# Files with NBA team logos, colors, etc
team_info <- read.csv(file.path(INPUT_DIR, "nba-colors-logos.csv"))
headshots <- read.csv(file.path(INPUT_DIR, "nba-headshots.csv"))
...
df <- readr::read_csv(file.path(INPUT_DIR, "player_level_data.csv"))
...
nba_team_data <-
  readr::read_csv(file.path(INPUT_DIR, "team_level_data.csv")) %>%
  select(-c("X1", "...1", ends_with("diff"))) %>%
  arrange(Teams, game_id)
...
df <- nba_team_data %>%
  ...
  select(-c(Opponent:`Point Differential Team`))
...
binned_means <- df %>% 
  reshape2::melt(id.vars = c("Teams", "game_id")) %>% # group by team/statistical measures
  ...
  dplyr::select(-c(game_id)) %>% # get rid of these now that we have game_cut (game_number in to check things)
  tidyr::complete(nesting(Teams, game_cut, variable), fill = list(n = 0)) %>% 
  dplyr::arrange(Teams, game_cut, variable) %>%  # fill/arrange/group_by 
  dplyr::group_by(Teams, game_cut, variable) %>% # Team, Grouping(game_cut), Variable
  # MAKE SURE THAT 'value' is ALL numeric values
  dplyr::mutate_at(.vars = vars("value"), .funs = as.numeric) %>%
...
lagged <- binned_means %>%
  tidyr::complete(nesting(Teams, game_cut, variable), fill = list(n = 0)) %>% # Set up Complete/Arrange on Team, Cut, Var
  dplyr::arrange(Teams, game_cut, variable) %>%
  dplyr::group_by(Teams, variable) %>% # take out categorical bin (game_cut)
...
list_box_score_metrics <- c("Points Scored", "Rim FG%", "Free Throw %", "Midrange FG%", "3PT FG%")
list_sq_metrics <- c("Points Expected", "Rim FG% Expected", "Free Throw % Expected", "Midrange FG% Expected", "3PT FG% Expected")
...
results %>%
  ...
  arrange(desc(r.squared)) %>%
  #slice(6:10) %>%
  dplyr::filter(variable %in% list_box_score_metrics) %>%
...
results %>%
  ...
  arrange(desc(r.squared)) %>%
  #slice(1:5) %>%
  dplyr::filter(variable %in% list_sq_metrics) %>%
  mutate(variable = reorder(variable, r.squared)) %>%
  ggplot(aes(r.squared, variable)) +
...
results %>%
  select(variable, r.squared) %>%
  arrange(desc(r.squared)) %>%
  dplyr::filter((variable %in% list_box_score_metrics) | (variable %in% list_sq_metrics)) %>%
  mutate(variable = reorder(variable, r.squared),
...
team_data <- readr::read_csv(file.path(INPUT_DIR, "team_level_data.csv"))
...
numerics <- team_data %>%
  select(is.numeric) %>%
  select(-c("X1", "...1", game_id)) %>%
  select(`Point Differential Team`, `Points Expected`:`Points Allowed`)

in_sample <- cor(numerics, use='pairwise.complete.obs') %>% 
  as.data.frame() %>% 
  select(`Point Differential Team`) %>% 
  mutate(rsq = (`Point Differential Team`)*(`Point Differential Team`)) %>% 
  arrange(desc(`rsq`))
...
binned_means <- team_data %>% 
  select(Teams, game_id, `Point Differential Team`, `Points Scored`) %>% # SELECT VARIABLE HERE.
  dplyr::group_by(Teams) %>% 
  dplyr::mutate(game_number = dplyr::row_number()) %>% # mutate in the running game number for that team/measure
  dplyr::ungroup() %>%
  dplyr::mutate(game_cut = cut(game_number, breaks = seq(0, 50, by = num))) %>%
  dplyr::group_by(Teams, game_cut) %>%
  summarise("Point Differential Mean" = mean(`Point Differential Team`),
...
# lag over each combination of team, bin, variable
lagged <- binned_means %>%
  dplyr::select(Teams, game_cut, `Point Differential Mean`, `Current`) %>% # now that we have bins, do not need game id or number
  tidyr::complete(nesting(Teams, game_cut), fill = list(n = 0)) %>% # Set up Complete/Arrange on Team & Cut
  dplyr::arrange(Teams, game_cut) %>%
  dplyr::group_by(Teams) %>% # take out game_cut
  dplyr::mutate(`Prev` = dplyr::lag(`Current`, default = NA, n = 1L)) %>% # lag
  dplyr::ungroup() %>%
...
results <- lagged2 %>%
  ...
  dplyr::mutate(fit = map(data, ~ lm(`Point Differential Mean` ~ Prev, data = .)),
                results = map(fit, broom::glance)) %>%
  tidyr::hoist(results, r_squared = "r.squared")

ret <- dplyr::bind_cols(results, yardstick::rsq(lagged2, truth = `Point Differential Mean`, estimate = Prev)) %>%
  mutate(var_name = "Points Scored") %>%
  select(var_name, r_squared, .estimate)

ret

#yardstick::rsq(lagged2, truth = `Point Differential Mean`, estimate = Prev)
```
...
predict_point_differential <- function(data, predictor, bin_size = 1) {
  # bin data based on team, game number
  bins_fun <- data %>% 
    select(Teams, game_id, `Point Differential Team`, predictor) %>% 
    dplyr::group_by(Teams) %>% 
    dplyr::mutate(game_number = dplyr::row_number()) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(game_cut = cut(game_number, breaks = seq(0, 100, by = bin_size))) %>%
    dplyr::group_by(Teams, game_cut) %>%
    summarise("Point Differential" = mean(`Point Differential Team`),
              Current = mean(!!sym(predictor)),
              .groups = "drop")

  lag <- bins_fun %>%
    dplyr::select(Teams, game_cut, `Point Differential`, Current) %>% # now that we have bins, do not need game id or number
    tidyr::complete(nesting(Teams, game_cut), fill = list(n = 0)) %>% # Set up Complete/Arrange on Team & Cut
    dplyr::arrange(Teams, game_cut) %>%
    dplyr::group_by(Teams) %>% # take out game_cut
  ...
  ret <- dplyr::bind_cols(res, yardstick::rsq(lag2, truth = `Point Differential`, estimate = `Prev`)) %>%
...
tib <- dplyr::tibble(
  ...
  "point_diff" = c(s1$`Point Differential Team`,
                   predict_point_differential(team_data, predictor = "Point Differential Team", bin_size = 1),
                   predict_point_differential(team_data, predictor = "Point Differential Team", bin_size = 4),
                   predict_point_differential(team_data, predictor = "Point Differential Team", bin_size = 8)),
  ...
  "rim" = c(s1$`Rim FG%`,
           predict_point_differential(team_data, predictor = "Rim FG%", bin_size = 1),
           predict_point_differential(team_data, predictor = "Rim FG%", bin_size = 4),
           predict_point_differential(team_data, predictor = "Rim FG%", bin_size = 8)),
  
  "pred_rim" = c(s1$`Rim FG% Expected`,
                predict_point_differential(team_data, predictor = "Rim FG% Expected", bin_size = 1),
                predict_point_differential(team_data, predictor = "Rim FG% Expected", bin_size = 4),
                predict_point_differential(team_data, predictor = "Rim FG% Expected", bin_size = 8)),
...
If you see mistakes or want to suggest changes, please [create an issue](https://github.com/MylesThomas/nba-scoring-stability-predictive-power/issues/new) on the source repository.

## Acknowledgments {.appendix}

Text and figures are licensed under Creative Commons Attribution [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). Source code is available at https://github.com/MylesThomas/nba-scoring-stability-predictive-power, unless otherwise noted. The figures that have been reused from other sources don't fall under this license and can be recognized by a note in their caption: "Figure from ...".